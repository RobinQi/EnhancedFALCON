...
#count finished jobs
    warn "Counting finished jobs in $i, this may take a while\n";
    ...
    #count finished jobs immediately under 0-raw_reads 1-preads_ovl or 2-asm_falcon
    opendir (DIR, $i) or die "Error: cannot read from dir $i\n";
    $donetotal += grep {m/_done$/ and -f File::Spec->catfile($i,$_)} readdir(DIR);
    closedir (DIR);
